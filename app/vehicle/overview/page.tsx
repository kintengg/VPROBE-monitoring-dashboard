...
function VehicleSurveillancePageContent() {
  ...
  const loadLocations = useCallback(async () => {
    ...
    try {
      const response = await getLocations({ domain: "vehicle" })
  ...
  const loadSummary = useCallback(async (date: string) => {
    try {
      const response = await getVehicleSummary(date || undefined)
      setSummary(response)
    } catch (error) {
      // Vehicle summary is optional — surface but don't block the page
      console.warn("Failed to load vehicle summary", error)
    }
  }, [])
  ...
  useEffect(() => {
    ...
    void Promise.all([loadLocations(), loadEvents(), loadSummary(selectedDate)])
  }, [loadEvents, loadLocations, loadSummary, selectedDate, settledUploadsVersion])
  ...
  const handleVideoModalChange = (open: boolean) => {
    ...
    if (!open) setSelectedUploadLocationId("")
  }
  ...
  useEffect(() => {
    if (searchParams.get("openAddVideo") !== "1") return
    handleOpenAddVideo()
  ...
  const handleSaveLocation = async (data: LocationPayload) => {
    try {
      setPageError(null)
      const payload: LocationPayload = { ...data, domain: "vehicle" }
      if (editingLocation) {
  ...
  const totals = useMemo(() => {
    const inGates = (summary?.perGateLos ?? []).filter((g) => g.flowGroup === "In").length
    const outGates = (summary?.perGateLos ?? []).filter((g) => g.flowGroup === "Out").length
    return { inGates, outGates }
  }, [summary])
          ...
          <div className="flex items-center gap-3 shrink-0 mr-6">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {summary?.totalGates ?? 0} gates · {totals.inGates} entrance · {totals.outGates} exit
                {summary?.dominantLos ? ` · LOS ${summary.dominantLos}` : ""}
              </p>
            </div>
          </div>
